// netlify/functions/get-community.mjs
...
export default async (req) => {
  ...
  try {
    ...
    if (!index.length) return new Response(JSON.stringify({ members: [] }), { status: 200, headers });

    const members = await Promise.all(
      index.slice(0, 200).map(async (id) => {
        try {
    ...
    return new Response(JSON.stringify({ members: members.filter(Boolean) }), { status: 200, headers });
  } catch(err) {
